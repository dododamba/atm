                            ...
                            <tr v-for="client in items.data">
                                ...
                                <td>
                                    <button v-if="client.compte.is_active == null " class="btn btn-success"
                                            @click="goToActive(client)"> Activer
                                    </button>
                                    <button v-else class="btn btn-danger" @click="goToActive(client)">
                                        Désactiver
                                    </button>
                                </td>
                                ...
                                <td class="text-right">
                                    <button type="button" rel="tooltip" class="btn btn-info btn-icon btn-sm "
                                            title="detail" @click="show(client)">
                                        <i class="fa fa-user"></i>
                                    </button>

                                    <button type="button" rel="tooltip" class="btn btn-danger btn-icon btn-sm "
                                            title="suppression" @click="deleteModal(client.compte.slug)">
                ...
                <div class="card-body">
                    ...
                    <form action="api/client-create" method="post">
                        ...
                        <div class="input-group mb-2">
                            ...
                            <input type="date" class="form-control" v-model="client.date_naissance">
                        </div>
                        ...
                        <div class="input-group mb-2">
                            ...
                            <select class="form-control" v-model="client.role">
...
<script>
    ...
    export default {
        ...
        methods: {
            list(url) {
                ...
                axios.get(url)
                    .then(function (response) {
                        app.items = response.data.content.clients;
            ...
            activation(c) {
                let ar = this;
                axios.get('api/client/activate/' + c.compte.slug)
                    .then(function (response) {
                        ar.message = response.data;
                        ar.success(ar.message.message);
                    });
                this.$refs['activation'].hide();
                this.list('/api/client?page=' + this.current_page_);
            },

            desactivation(c) {
                let ar = this;
                axios.get('api/client/desactivate/' + c.compte.slug)
                    .then(function (response) {
                        ar.message = response.data;
                        ar.success(ar.message.message);
                    });
            ...
            destroy(slug) {
                let app = this;
                axios.delete('/api/delete-client/' + slug)
                    .then(function (response) {
            ...
            storeModal() {
                var app = this;
                var newPersonnel = app.client;
                axios.post('/api/create/client', newPersonnel)
                    .then(function (response) {
                        app.message_ = response.data.message;
                        app.status = response.data.status;
                        app.validations = response.data.content;
                        console.log(app.validations);
                    }).catch(function (response) {
                });
